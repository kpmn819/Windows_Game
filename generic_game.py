...
def choose_game():
    ...
    if type_ == 'picture':
        #Get the background and picture data
        background = make_surface(gpath + type_ + '_bkg.jpg', scale_to_screen=True)
        # get the picture data from csv file
        curr_game = PictGame( name, background, get_file(type_ + '_picture.csv', 2)[0], [0,0])
    elif type_ == 'text':
        background = make_surface(gpath + type_ + '_bkg.jpg', scale_to_screen=True)
        curr_game = TextGame( name, background, get_file(type_ + '_qna.csv', 4)[0], [0,0])
